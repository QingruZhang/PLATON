...
def train_config(parser):
    ...
    #
    parser.add_argument('--beta3', default=0.99, type=float)
    parser.add_argument('--save_record', action='store_true')
    parser.add_argument('--pruning_ratio', default=0, type=float)
    return parser
...
def main():
    ...
    train_data_list.append(multi_task_train_data)
    ...
    for idx, dataset in enumerate(args.train_datasets):
        data = train_data_list[idx] # data_list[idx] is a DataLoader object
        for (batch_info, batch_data) in data:
            batch_info, batch_data = Collater.patch_data(device, batch_info, batch_data)
            loss = model.predict_loss(batch_info, batch_data)
            if args.fp16:
        ...
        model_file = os.path.join(output_dir, 'ipt.pt')
        torch.save(is_dict, model_file)
        all_is = torch.cat([is_dict[n].view(-1) for n in is_dict])
        mask_threshold = torch.kthvalue(all_is, int(all_is.shape[0]*(1-args.pruning_ratio)))[0].item()
        for n,p in model.network.named_parameters():
            if any(nd in n for nd in mask_param_name):
    ...
    ################# Evaluation ###################
    evaluation(model, args.test_datasets, dev_data_list, task_defs, output_dir, -1, with_label=True, tensorboard=tensorboard, glue_format_on=args.glue_format_on, test_on=False, device=device, logger=logger)
    evaluation(model, args.test_datasets, test_data_list, task_defs, output_dir, -1, with_label=args.with_label, tensorboard=tensorboard, glue_format_on=args.glue_format_on, test_on=True, device=device, logger=logger)
    print_message(logger, '[new test scores at {} saved.]'.format(-1))
