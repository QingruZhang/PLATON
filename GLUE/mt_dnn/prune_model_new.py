...
from module.bert_optim import Adamax, RAdam, AdamW, StructAwareAdamW, StructAwareAdamax, UnstructAwareAdamax
...
class MTDNNModel(object):
    def __init__(self, opt, device=None, state_dict=None, num_train_step=-1):
        ...
        if self.config['optimizer'] in ['sadamw', 'sadamax'] :
            optimizer_parameters = self._get_sadam_param_groups()
        elif self.config['optimizer'] in ['usadamax', 'usadamax_const_lr'] :
    ...
    def _get_usadam_param_groups(self):
        no_decay = ['bias', 'gamma', 'beta', 'LayerNorm.bias', 'LayerNorm.weight']
        ...
        # mask_params = [(n, p) for n, p in self.network.named_parameters() if 'embedding' not in n]
        # common_params = [(n, p) for n, p in self.network.named_parameters() if 'embedding' in n]
    ...
    def _get_sadam_param_groups(self):
        no_decay = ['bias', 'gamma', 'beta', 'LayerNorm.bias', 'LayerNorm.weight']
        head = ['attention.self', 'attention.output.dense.weight']
        ffn = ['output.dense', 'intermediate.dense']

        head_params = [(n, p) for n, p in self.network.named_parameters() if any(nd in n for nd in head) and 'mask' not in n]
        ffn_params = [(n, p) for n, p in self.network.named_parameters() if any(nd in n for nd in ffn) and 'mask' not in n and 'attention' not in n]
        common_params = [(n, p) for n, p in self.network.named_parameters() if not any(nd in n for nd in ffn) and not any(nd in n for nd in head) and 'mask' not in n]
        ...
        optimizer_parameters = [
            ...
            {'params': [p for n, p in head_params if not any(nd in n for nd in no_decay)],
             'ipts_params': [p for n, p in self.network.named_parameters() if 'heads_mask' in n],
             'weight_decay': 0.01,
             'params_type': 'head'},
            # [out, in]
            # 'bert.encoder.layer.11.attention.self.query.weight', [768 (head_dim*n_head), 768]
            # 'bert.encoder.layer.11.attention.self.key.weight', [768 (head_dim*n_head), 768]
            # 'bert.encoder.layer.11.attention.self.value.weight', [768 (head_dim*n_head), 768]
            # 'bert.encoder.layer.11.attention.output.dense.weight', [768, 768(head_dim*n_head)]
            {'params': [p for n, p in ffn_params if not any(nd in n for nd in no_decay)],
             'ipts_params': [p for n, p in self.network.named_parameters() if 'ffn_mask' in n],
             'weight_decay': 0.01,
             'params_type': 'ffn'},
            #'bert.encoder.layer.11.intermediate.dense.weight', [768*3, 768]
            #'bert.encoder.layer.11.output.dense.weight', [768, 768*3]
            {'params': [p for n, p in common_params if any(nd in n for nd in no_decay)],
             'weight_decay': 0,
             'params_type': 'common'},
            {'params': [p for n, p in head_params if any(nd in n for nd in no_decay)],
             ...
             'params_type': 'head'},
            # [out]
            # 'bert.encoder.layer.11.attention.self.query.bias', [768(head_dim*n_head),]
            # 'bert.encoder.layer.11.attention.self.key.bias', [768(head_dim*n_head),]
            # 'bert.encoder.layer.11.attention.self.value.bias' [768(head_dim*n_head),]
            {'params': [p for n, p in ffn_params if any(nd in n for nd in no_decay)],
             'ipts_params': [p for n, p in self.network.named_parameters() if 'ffn_mask' in n],
             'weight_decay': 0,
             'params_type': 'ffn'},
            #'bert.encoder.layer.11.intermediate.dense.bias', [768*3]
    ...
    def _setup_optim(self, optimizer_parameters, state_dict=None, num_train_step=-1):
        ...
        if self.config['optimizer'] == 'sgd':
            ...
            self.optimizer = AdamW(optimizer_parameters,
                                  ...
                                  weight_decay=self.config['weight_decay'])
            if self.config.get('have_lr_scheduler', False): self.config['have_lr_scheduler'] = False
        elif self.config['optimizer'] == 'sadamw':
            self.optimizer = StructAwareAdamW(optimizer_parameters,
                                  lr=self.config['learning_rate'],
                                  warmup=self.config['warmup'],
                                  t_total=num_train_step,
                                  max_grad_norm=self.config['grad_clipping'],
                                  ...
                                  weight_decay=self.config['weight_decay'])
            if self.config.get('have_lr_scheduler', False): self.config['have_lr_scheduler'] = False
        elif self.config['optimizer'] == 'sadamax':
            self.optimizer = StructAwareAdamax(optimizer_parameters,
                                    self.config['learning_rate'],
                                    warmup=self.config['warmup'],
                                    t_total=num_train_step,
            ...
            self.optimizer = UnstructAwareAdamax(optimizer_parameters,
                                    ...
                                    weight_decay=self.config['weight_decay'])
            if self.config.get('have_lr_scheduler', False): self.config['have_lr_scheduler'] = False
        elif self.config['optimizer'] == 'usadamax_const_lr':
            self.optimizer = UnstructAwareAdamax(optimizer_parameters,
                                    self.config['learning_rate'],
                                    warmup=self.config['warmup'],
                                    t_total=-1,
    ...
    def update(self, batch_meta, batch_data):
        ...
        if self.local_updates % self.config.get('grad_accumulation_step', 1) == 0:
            ...
            if self.config['save_record'] and (1000 <= self.updates < 2000):
                self.optimizer.step(update_record=True)
                record_file = os.path.join(self.config['output_dir'], 'record_' + str(int(self.updates)-1000) + '.pt')
                self.save_record(record_file)
            else:
                self.optimizer.step()

            self.update_exp_avg_ipt()
            self.optimizer.zero_grad()

    def update_exp_avg_ipt(self):
        # mask_param_name = ['attention.self', 'attention.output.dense', 'output.dense', 'intermediate.dense']
        non_mask_name = ['embedding', 'LayerNorm']
    ...
    def predict_loss(self, batch_meta, batch_data):
    ...
    def save_record(self, filename):
        modulated_lrs, ipt_vars, ipts = self.optimizer.get_sensitivity_record()
        params = {
            'modulated_lrs': modulated_lrs,
            'ipt_vars': ipt_vars,
            'ipts': ipts
        }
        torch.save(params, filename)
        logger.info('sensitivity record saved to {}'.format(filename))
        self.optimizer.init_sensitivity_record()
