...
class MTDNNModel(object):
    def __init__(self, opt, device=None, state_dict=None, num_train_step=-1):
        ...
        if self.config['local_rank'] != -1:
            self.mnetwork = torch.nn.parallel.DistributedDataParallel(self.network, 
    ...
    def update(self, batch_meta, batch_data):
        ...
        if self.local_updates % self.config.get('grad_accumulation_step', 1) == 0:
            ...
            if self.config['save_record'] and (1000 <= self.updates < 2000):
                ...
                self.optimizer.step()
            if self.config['deltaT'] <= 0:
                # Update I only
                self.update_exp_avg_ipt()
            else:
                # Update I and U with the local window 
                self.update_ipt_with_local_windiw()
            self.optimizer.zero_grad()
    ...
    def update_exp_avg_ipt(self):
        ...
        for n,p in self.network.named_parameters():
            if any(nd in n for nd in mask_param_name):
                ...
                if self.config["beta1"] > 0 and self.config["beta1"]<1:
                    # Update I 
                    self.exp_avg_ipt[n] = self.config["beta1"] * self.exp_avg_ipt[n] + \
                                          (1 - self.config["beta1"]) * self.ipt[n]
                else:
                    raise ValueError("Incorrect beta1.")

    def update_ipt_with_local_windiw(self):
        mask_param_name = ['attention.self', 'attention.output.dense', 'output.dense', 'intermediate.dense']
        for n,p in self.network.named_parameters():
            if any(nd in n for nd in mask_param_name):
                if n not in self.exp_avg_ipt:
                ...
                local_step = self.updates % self.config['deltaT']
                update_step = self.updates // self.config['deltaT']
                if local_step == 0: 
                    # Update I 
                    self.exp_avg_ipt[n] = self.config["beta1"] * self.exp_avg_ipt[n] + \
                                                (1 - self.config["beta1"]) * self.ipt[n]
                    if self.config['beta2'] >= 0 and self.config['beta2'] < 1: 
                        ...
                        raise ValueError("Incorrect beta2.")
                    self.ipt[n] = (p * p.grad).abs().detach()
                else:
                    # Calculate the current sensitivity 
